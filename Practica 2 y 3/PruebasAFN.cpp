...
void PruebasAFN::crearAFN()
{
    ...
    { // Se piden los datos del AFN por consola
...
AFN PruebasAFN::convertirERaAFN(string expresionRegular)
{
    ...
    { // Se encuentra un parentesis
      ...
      balanceo++; // Incrementamos el contador de balanceo
        ...
        {  // Buscamos el parentesis de cierre
            if(expresionRegular[j] == ')')
            {
                balanceo--;
                if(balanceo == 0)
                { // Si ya esta balanceado
        ...
        {
            if(expresionRegular[j] == '*')
            { // Si a lo que esta entre parentesis le sigue una cerradura de Kleen
        ...
        else
        {
            ...
            { // Si el numero del estado inicial del subAfn no es cero
        ...
        { // Si no es ninguno de los operadores de ER
            bool esEpsilon = false; // Bandera para saber si es una transicion epsilon
            if(expresionRegular[i]=='E')
            { // Si el caracter corresponde a epsilon
                esEpsilon = true;
            }
            if(i < cadlen - 1 )
            {
                if(expresionRegular[i+1]=='*')
                { // Si el siguiente caracter es una cerradura de Kleen
                ...
                {  // Si el siguiente caracter es una cerradura positiva
                    afn.aplicarCerraduraPositiva(&afn,expresionRegular[i],esEpsilon);
                    afn.agregarSimboloAlAlfabeto(&afn,expresionRegular[i]);
                    i+=2;
                }
                else
                { // Si no es cerradura, solo se concatena
